...
class DataSource:
    ...
    def getActivityMinsByStateRange(self, state, activity, start, end):
        '''
        Returns a list of average minutes spent on the specified activity per day in the specified State.
        ...
        '''
        if (end == start):
            average = self.getActivityAverageMinsByYearAndState(start, state, activity)
            return average
        elif (start > end):
            temp = start
            start = end
    ...
    def getNumberOfPeopleSurveyedByStateAverage(self, state, start, end):
        '''
        Returns a count of people surveyed on the specified year in the specified State.
        ...
        RETURN:
            a count of people surveyed on the specified year in the specified State.
        '''
        averagePeopleSurveyed = 0
        if (start > end):
            temp = start
            start = end
            end = temp
        for year in range(start, end + 1):
            averagePeopleSurveyed += self.getNumberOfPeopleSurveyedByStateAndYear(state, year)
        return round(averagePeopleSurveyed/((end + 1)-start))
